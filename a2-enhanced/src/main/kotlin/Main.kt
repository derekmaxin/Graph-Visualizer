...
val root = BorderPane() //global root for easy re-rendering
...
object Model {
    ...
    //ENHANCEMENTS
    var curActivePieColour = "Red"

    fun setActivePieColourScheme(colour : String) {
        curActivePieColour = colour
        refresh(activeView)
    }

    fun getActivePieColourScheme() : String {
        return curActivePieColour
    }
}
...
class SemGraphView() : Canvas() {
    init {
        ...
        // median enhancement
        var sdCalc = 0.0
        var myArray : DoubleArray = Model.getActiveDataSetValues().copyOf()
        myArray.sort()
        val median = myArray[myArray.size/2]
        ...
        this.apply{
            ...
            graphicsContext2D.apply{
                ...
                strokeLine(
                    10.0,
                    canvasHeight - (scalingRatio * (mean - minVal)) - (xDist / 3) ,
                    canvasWidth - 10.0,
                    canvasHeight - (scalingRatio * (mean - minVal)) - (xDist / 3)
                )
                //upper SEM line
                ...
                strokeLine(
                    10.0,
                    canvasHeight - (scalingRatio * (mean + semCalc - minVal)) - (xDist / 3) ,
                    canvasWidth - 10.0,
                    canvasHeight - (scalingRatio * (mean + semCalc - minVal)) - (xDist / 3)
                )
                //lower SEM line
                ...
                //median line
                setLineDashes(2.0, 20.0)
                stroke = Color.BLUE
                strokeLine(
                    10.0,
                    canvasHeight - (scalingRatio * (median + sdCalc - minVal)) - (xDist / 3) ,
                    canvasWidth - 10.0,
                    canvasHeight - (scalingRatio * (median + sdCalc - minVal)) - (xDist / 3)
                )
                ...
                fillText("(yellow) mean: " + mean + "\n",  10.0, 10.0, 1000.0)
                stroke = Color.GREEN
                fillText("(green) SEM: " + semCalc + "\n", 10.0, 25.0, 1000.0)
                stroke = Color.BLUE
                fillText("(blue) median: " + median, 10.0, 40.0, 1000.0)
            }
        }
    ...
    init {
        ...
        Model.getActiveDataSetValues().forEachIndexed { index, element ->
            val arc = Arc().apply{
                ...
                type = ArcType.ROUND
                if (Model.getActivePieColourScheme() == "Red") fill = Color.color(index * curColor,0.0,0.0)
                else if (Model.getActivePieColourScheme() == "Green") fill = Color.color(0.0, index * curColor,0.0)
                else fill = Color.color(0.0, 0.0, index * curColor)
...
class ToolBarController() : ToolBar() {
    init {
        ...
        val barSEMView = RadioButton("Bar (STAT)").apply{
            toggleGroup = graphViewButtonToggleGroup
            prefWidth = 75.0
        ...
        //SEPERATOR
        val separator3 = Separator().apply{
            padding = Insets(0.0, 5.0, 0.0, 5.0)
        }
        this.items.add(separator3)

        //PIE CHART COLOuR PICKER ENHANCEMENT
        this.items.add(Label("Pie Colour: "))

        val colourPickerComboBox: ComboBox<String> = ComboBox()
        colourPickerComboBox.items.addAll(
            "Red", "Green", "Blue"
        )
        colourPickerComboBox.promptText = Model.getActivePieColourScheme()
        this.items.add(colourPickerComboBox)

        colourPickerComboBox.onAction = EventHandler {
            Model.setActivePieColourScheme(colourPickerComboBox.value)
        }
    }
